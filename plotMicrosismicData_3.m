...
distanceThreshold = 100; %m


%% INITIALIZATION
process_stages = [6 7 8 13 14];
...
for stage_i = stages
    ...
    %% process data
    isLower = calculateDistances([X Y Z],data,stage_i,distanceThreshold);

    X = X(~isLower); Y = Y(~isLower); Z = Z(~isLower);
    UX = UX(~isLower); UY = UY(~isLower); UZ = UZ(~isLower);
    DIP = DIP(~isLower); STRIKE = STRIKE(~isLower);
...
function isLower = calculateDistances(actualStage, data,stage_i,threshold)
   
if stage_i ~= 1
    previousStage = stage_i - 1;
    if previousStage == 1
        logvec = data.STAGE == 1;
    else
        logvec = any( (data.STAGE == previousStage:-1:1)' );
    end
    oldStage = [data.X(logvec),data.Y(logvec),data.Z(logvec)];

    distances = sqrt(sum((permute(actualStage, [1 3 2]) - permute(oldStage, [3 1 2])).^2, 3)); 
    % se pude reemplazar por pdist2 con el toolbox de statistic and ML
    % The resulting distances matrix has size num_points_actual x
    % num_points_old, distances(i,j) is distance between actual point i and
    % old point j

    isLower = distances < threshold;
    isLower = any(isLower');
%     disp(sum(isLower));
else
    [n,~] = size(actualStage);
