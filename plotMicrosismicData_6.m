...
stages = [1 5 8 10 12 15]; % plotea todas
% stages = [1 6 7 8 13 14]; % plotea solo 1 3 y 5
switchPlot = 'D/S'; % 'U' 'D/S' 'both'
plotFilterPLanes = true; deltaZ = 200;
plotPlanes = false;
...
%% CASING PLOT
plot3(f1,casingData.EWm+locationGrid(1)-meanStage1(1),casingData.NSm+locationGrid(2)-meanStage1(2),casingData.TVDm,'o')
...
for i = stages
    ...
    plot3(f1,casingMatrix(idx,1),casingMatrix(idx,2),casingMatrix(idx,3),'*','Color',Colors(count,:));
    count = count + 1;
